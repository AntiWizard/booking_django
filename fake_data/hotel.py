...
def insert_room(conn, info, price_ids, flag=True):
    ...
    if flag and conn.status == 1:
        ...
        for d in data:
            cursor.execute(
                "INSERT INTO hotel_hotelroom(number, capacity, description, status, is_valid, created_time, modified_time, avatar, hotel_id, price_id)"
                "VALUES (%s,%s,%s,%s,True,now(),now(),%s,%s,%s)", d)
